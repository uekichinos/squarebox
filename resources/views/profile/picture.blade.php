    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-12 gap-4 mb-3">
                <div class="col-span-6 text-xl text-gray-800">{{ __('Update Picture') }}</div>
                <div class="col-span-6 space-x-8 sm:-my-px sm:ml-10 text-right"></div>
            </div>
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">

                    <form method="POST" action="{{ route('picture.update', $user->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-12 gap-4">
                            ...
                            <div class="col-span-9">
                                ...
                                @endif
                                <div class="mb-10">
                                    <img src="{{ asset('storage/img/avatars/'.$user->picture) }}" style="width:200px; height:200px;">
                                </div>
                                <div class="mb-10">
                                    <label class="block text-gray-700 text-sm mb-2" for="picture">
                                        Picture
                                    </label>
                                    ...
                                    @error('picture')
                                        <div notify="picture" class="bg-red-100 border-l-4 border-red-500 text-red-700 py-1 pl-2 my-2 text-sm">{{ $message }} <span class="cursor-pointer float-right mr-2" onclick="closeAlert('picture')">X</span></div>
                                    @enderror
                                </div>
                            </div>
                        </div>
